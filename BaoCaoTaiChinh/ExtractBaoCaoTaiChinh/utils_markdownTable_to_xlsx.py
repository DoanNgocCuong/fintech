...
def _remove_last_column(df: "pd.DataFrame") -> "pd.DataFrame":
    """
    Return a copy of dataframe without the last column.

    Keep the original dataframe when it has zero or one column
    to avoid creating empty sheets.
    """
    if pd is None or df is None or df.shape[1] <= 1:
        return df
