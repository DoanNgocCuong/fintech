...
Usage:
    python main_JSONLuuChuyenTienTe_to_DB.py <json_file_or_folder> [--no-overwrite] [--no-recursive]
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python main_JSONLuuChuyenTienTe_to_DB.py <json_file_or_folder> [--no-overwrite] [--no-recursive]")
        print("\nOptions:")
