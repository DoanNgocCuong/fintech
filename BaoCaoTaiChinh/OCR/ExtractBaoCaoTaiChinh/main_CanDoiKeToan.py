...
def process_balance_sheet(
    ...
    4. Đi qua từng trang và kiểm tra từng trang có chứa "Bảng Cân Đối Kế Toán" không
    ...
    LƯU Ý:
    - Hàm kiểm tra "Bảng Cân Đối Kế Toán" trên TỪNG TRANG, không kiểm tra toàn bộ file
    ...
    with pd.ExcelWriter(str(output_path), engine='openpyxl') as writer:
        sheet_count = 0
        # Quét qua từng trang để tìm và extract các bảng của Bảng Cân Đối Kế Toán
        # Xử lý Bảng Cân Đối Kế Toán - QUÉT QUA TỪNG TRANG
        print("\n" + "-" * 80)
        print("Processing: BANG CAN DOI KE TOAN (Balance Sheet)")
        print("-" * 80)
        try:
            # Quét qua từng trang để tìm và extract các bảng của Bảng Cân Đối Kế Toán
            # Sử dụng hàm generic process_pages_for_financial_statements với detect_candoiketoan
    ...
    if create_json and sheet_count > 0:
        ...
        print("-" * 80)
        try:
            # Lazy import để tránh circular import (utils_xlsx_to_json import từ main_CanDoiKeToan)
...
def main():
    ...
    Nếu input là folder, sẽ xử lý tất cả file .md trong folder đó.
    ...
    if input_path_obj.is_file():
        ...
        md_files = list(input_path_obj.glob("*.md"))
        if not md_files:
            print(f"No .md files found in folder: {input_path}")
            sys.exit(1)
        print(f"Found {len(md_files)} .md file(s) in folder: {input_path}")
    else:
    ...
    for idx, md_file in enumerate(md_files, 1):
        ...
        try:
            ...
            print(f"\n✓ Successfully processed: {md_file.name}")
            print(f"  Output file: {result_path}")
            successful_files.append((md_file.name, result_path))
        except Exception as e:
